...
function validateSettings() {
  const baseUrl = validateBaseUrl(els.baseUrl.value);
...
async function handleAddMacro() {
  ...
  try {
    normalizedSequence = normalizeDtmfSequence(sequence);
  } catch (error) {
    setStatus(error.message, "error");
    return;
  }
  state.dtmfMacros.push({ label, sequence: normalizedSequence });
